...
enum combo_events {
    ...
    OSM_SHIFT_COMBO, // SPACE and SYMBOL => activate QMK one-shot shift.
    COMBO_LENGTH
};
...
const uint16_t osm_shift_combo[] PROGMEM = {KC_SPACE, SYMBOL, COMBO_END};

combo_t key_combos[] = {
    [CAPS_LOCK_COMBO] = COMBO_ACTION(caps_lock_combo),
    [CAPS_WORD_COMBO] = COMBO_ACTION(caps_word_combo),
    [OSM_SHIFT_COMBO] = COMBO_ACTION(osm_shift_combo),
};
...
layer_state_t layer_state_set_user(layer_state_t state) {
    ...
    //     update_tri_layer_state(state, _MOTION, _NUMBER, _EXTEND));
}
