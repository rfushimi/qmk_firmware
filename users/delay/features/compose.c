 ...
 */
bool _compose_handle_keypress(compose_state_t *state, uint16_t keycode, keyrecord_t *record) {
...
bool process_record_compose(compose_state_t *state, uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        case COMPOSE:
...
void matrix_scan_compose(compose_state_t *state) {
    if (state->is_composing && timer_elapsed(state->timer) > (uint16_t)COMPOSE_TERM) {
        _compose_process_queue(state);
    }
}

bool compose_queue_equal(compose_state_t *state, uint16_t keycodes[COMPOSE_KEYCODE_QUEUE_SIZE]) {
    for (int i = 0; i < COMPOSE_KEYCODE_QUEUE_SIZE; ++i) {
