    ...
    switch (keycode) {
        ...
        case KM_FIRST_DEFAULT_LAYER ... KM_LAST_DEFAULT_LAYER:
            if (record->event.pressed) {
                ...
                const uint8_t layer = keycode - KM_FIRST_DEFAULT_LAYER + LAYER_ALPHAS_FIRST;
#else
                // If ctrl is held, use the HR-less version of the layout.
                const uint8_t layer = keycode - KM_FIRST_DEFAULT_LAYER + (mods & MOD_MASK_CTRL ? LAYER_NO_MODS_ALPHAS_FIRST : LAYER_ALPHAS_FIRST);
