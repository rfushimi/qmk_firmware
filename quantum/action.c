...
__attribute__((weak)) bool get_ignore_mod_tap_interrupt(uint16_t keycode,
                                                        keyrecord_t *record) {
  return false;
}
#endif
...
__attribute__((weak)) bool get_retro_tapping(uint16_t keycode,
                                             keyrecord_t *record) {
  return false;
}
#endif
...
#ifndef NO_ACTION_TAPPING
/** \brief Allows for handling tap-hold actions immediately instead of waiting
 * for TAPPING_TERM or another keypress.
 *
 * FIXME: Needs documentation.
 */
...
void register_button(bool pressed, enum mouse_buttons button) {
  ...
  currentReport.buttons = pressed ? currentReport.buttons | button
...
static void bilateral_combinations_hold(action_t action, keyevent_t event) {
  ...
  bilateral_combinations.mods = (action.kind.id == ACT_LMODS_TAP)
                                    ? action.key.mods
                                    : action.key.mods << 4;
...
#if (BILATERAL_COMBINATIONS + 0)
      if (TIMER_DIFF_16(event.time, bilateral_combinations.time) >
...
void process_action(keyrecord_t *record, action_t action) {
  ...
  switch (action.kind.id) {
      ...
      if (event.pressed) {
        if (mods) {
          if (IS_MOD(action.key.code) || action.key.code == KC_NO) {
            // e.g. LSFT(KC_LGUI): we don't want the LSFT to be weak as it would
            // make it useless. This also makes LSFT(KC_LGUI) behave exactly the
            // same as LGUI(KC_LSFT). Same applies for some keys like KC_MEH
            // which are declared as MEH(KC_NO).
            add_mods(mods);
          } else {
            add_weak_mods(mods);
          }
    ...
    defined(IGNORE_MOD_TAP_INTERRUPT_PER_KEY)
              if (
#ifdef IGNORE_MOD_TAP_INTERRUPT_PER_KEY
                  !get_ignore_mod_tap_interrupt(
                      get_event_keycode(record->event, false), record) &&
#endif
...
#endif
          register_button(event.pressed,
                          MOUSE_BTN_MASK(action.key.code - KC_MS_BTN1));
          break;
#endif
        default:
          mousekey_send();
    ...
    case ACT_LAYER:
      if (action.layer_bitop.on == 0) {
        ...
        if (!event.pressed) {
          ...
          layer_state_t bits = ((layer_state_t)action.layer_bitop.bits)
                               << shift;
          layer_state_t mask =
    ...
    case ACT_MACRO:
      action_macro_play(
          action_get_macro(record, action.func.id, action.func.opt));
      break;
#endif
#ifdef SWAP_HANDS_ENABLE
    case ACT_SWAP_HANDS:
  ...
  if (!is_tap_action(action)) {
    ...
    if (event.pressed) {
      ...
      if (tap_count > 0) {
        ...
        if (
#ifdef RETRO_TAPPING_PER_KEY
            get_retro_tapping(get_event_keycode(record->event, false),
                              record) &&
#endif
...
#ifdef SWAP_HANDS_ENABLE
#ifndef NO_ACTION_ONESHOT
  if (event.pressed && !(action.kind.id == ACT_SWAP_HANDS &&
                         action.swap.code == OP_SH_ONESHOT)) {
 ...
 * \param delay The amount of time in milliseconds to leave the keycode
 ...
 *
 * \param code The basic keycode to tap. If `code` is `KC_CAPS`, the delay will
 * be `TAP_HOLD_CAPS_DELAY`, otherwise `TAP_CODE_DELAY`, if defined.
 */
void tap_code(uint8_t code) {
  tap_code_delay(code, code == KC_CAPS ? TAP_HOLD_CAPS_DELAY : TAP_CODE_DELAY);
}

/** \brief Adds the given physically pressed modifiers and sends a keyboard
 * report immediately.
 *
...
/** \brief Removes the given physically pressed modifiers and sends a keyboard
 * report immediately.
 *
...
/** \brief Adds the given weak modifiers and sends a keyboard report
 * immediately.
 *
 * \param mods A bitfield of modifiers to register.
 */
...
/** \brief Removes the given weak modifiers and sends a keyboard report
 * immediately.
 *
 * \param mods A bitfield of modifiers to unregister.
 */
...
void debug_event(keyevent_t event) {
  dprintf("%04X%c(%u)", (event.key.row << 8 | event.key.col),
          (event.pressed ? 'd' : 'u'), event.time);
}
/** \brief Debug print (FIXME: Needs better description)
 *
 * FIXME: Needs documentation.
